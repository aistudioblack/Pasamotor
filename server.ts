...
`;
  ...
  app.get('/api/cron/weekly-blog', async (req, res) => {
    const authHeader = req.headers.authorization;
    ...
    const WEEKLY_TOPICS = [
      { title: 'Honda PCX 125 Periyodik Bakım Rehberi 2026', keyword: 'honda pcx 125 bakım', brand: 'honda' },
      { title: 'Yamaha NMAX Yedek Parça Fiyatları 2026', keyword: 'yamaha nmax yedek parça', brand: 'yamaha' },
      { title: 'TVS Apache RTR 160 Bakım ve Servis Rehberi 2026', keyword: 'tvs apache rtr 160 bakım', brand: 'tvs' },
      { title: 'Motosiklet Zincir Bakımı Nasıl Yapılır? 2026', keyword: 'motosiklet zincir bakımı', brand: null },
      { title: 'Hero Splendor Plus Yedek Parça Rehberi 2026', keyword: 'hero splendor plus yedek parça', brand: 'hero' },
      { title: 'Vespa Sprint 125 İstanbul Yedek Parça 2026', keyword: 'vespa sprint 125 yedek parça istanbul', brand: 'vespa' },
      { title: 'CFMOTO 300NK Servis ve Bakım Rehberi 2026', keyword: 'cfmoto 300nk servis', brand: 'cfmoto' },
      { title: 'Motosiklet Akü Değişimi Ne Zaman Gerekir? 2026', keyword: 'motosiklet akü değişimi', brand: null }
    ];

    const weekNumber = Math.floor(Date.now() / (7 * 24 * 60 * 60 * 1000));
    const topic = WEEKLY_TOPICS[weekNumber % WEEKLY_TOPICS.length];

    try {
      const openrouterKey = process.env.VITE_OPENROUTER_API_KEY;
      if (!openrouterKey) throw new Error('OpenRouter key eksik');

      const prompt = `
Sen Paşa Motor'un (İstanbul Fatih yetkili motosiklet servisi) blog yazarısın. Aşağıdaki konuda Türkçe SEO blog yazısı yaz.

Konu: ${topic.title}
Hedef kelime: ${topic.keyword}
...
- Sonunda SSS bölümü (4 soru-cevap)
- Son paragrafta: "WhatsApp sipariş hattı: wa.me/905348996817"
- Marka sesi: "Paşa Motor olarak..."
- Fatih/İstanbul vurgusu yap
- slug: ${topic.keyword.toLowerCase().replace(/\s+/g, '-').replace(/[^a-z0-9-]/g, '')}
`;

      const articleRes = await fetch('https://openrouter.ai/api/v1/chat/completions', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${openrouterKey}`,
          'HTTP-Referer': 'https://pasamotor.com.tr',
          'X-Title': 'Paşa Motor Cron Blog'
        },
        body: JSON.stringify({
      ...
      fetch('http://127.0.0.1:3000/api/github/push', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${process.env.CRON_SECRET}` },
        body: JSON.stringify({ message: `content: haftalık blog - ${topic.title}` })
      }).catch(console.error);

      return res.json({ success: true, title: topic.title, message: 'Blog yazısı taslak olarak oluşturuldu' });
    } catch (err) {
      console.error('Cron blog hatası:', err);
