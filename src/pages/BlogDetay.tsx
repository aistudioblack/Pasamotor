...
import { 
  ...
  CheckCircle2, User, Award, List, Heart, Sparkles, AlertCircle, Bookmark, Copy, Type
} from "lucide-react";
...
const BlogDetay = () => {
  ...
  const [showFloatingSizeControls, setShowFloatingSizeControls] = useState(false);
  const contentRef = useRef<HTMLDivElement>(null);

  const getParagraphSizeClass = (sz: string) => {
    switch (sz) {
      case "sm": return "text-[14px] leading-6 md:text-[15px] md:leading-7";
      case "lg": return "text-[18px] leading-8 md:text-[19px] md:leading-9";
      case "xl": return "text-[21px] leading-9 md:text-[22px] md:leading-10";
      case "base":
      default: return "text-base leading-8";
    }
  };

  const getListSizeClass = (sz: string) => {
    switch (sz) {
      case "sm": return "text-[14px] leading-6 md:text-[15px] md:leading-7";
      case "lg": return "text-[18px] leading-8 md:text-[19px] md:leading-9";
      case "xl": return "text-[21px] leading-9 md:text-[22px] md:leading-10";
      case "base":
      default: return "text-base leading-7";
    }
  };

  const getHeadingSizeClasses = (sz: string) => {
    switch (sz) {
      case "sm": return { 
        h1: "text-2xl md:text-3xl font-bold mb-5 mt-7 text-foreground scroll-mt-24", 
        h2: "text-xl md:text-2xl font-semibold mb-3 mt-7 text-foreground border-b pb-2 scroll-mt-24", 
        h3: "text-lg md:text-xl font-semibold mb-2.5 mt-5 text-foreground scroll-mt-24" 
      };
      case "lg": return { 
        h1: "text-4xl md:text-5xl font-bold mb-7 mt-9 text-foreground scroll-mt-24", 
        h2: "text-3xl md:text-4xl font-semibold mb-5 mt-9 text-foreground border-b pb-2 scroll-mt-24", 
        h3: "text-2xl md:text-3xl font-semibold mb-3.5 mt-7 text-foreground scroll-mt-24" 
      };
      case "xl": return { 
        h1: "text-5xl md:text-6xl font-bold mb-8 mt-10 text-foreground scroll-mt-24", 
        h2: "text-4xl md:text-5xl font-semibold mb-6 mt-10 text-foreground border-b pb-2 scroll-mt-24", 
        h3: "text-3xl md:text-4xl font-semibold mb-4 mt-8 text-foreground scroll-mt-24" 
      };
      case "base":
      default: return { 
        h1: "text-3xl font-bold mb-6 mt-8 text-foreground scroll-mt-24", 
        h2: "text-2xl font-semibold mb-4 mt-8 text-foreground border-b pb-2 scroll-mt-24", 
        h3: "text-xl font-semibold mb-3 mt-6 text-foreground scroll-mt-24" 
      };
    }
  };
  ...
  useEffect(() => {
    const onScroll = () => {
      ...
      setShowFloatingSizeControls(scrolled > 400);
    };
  ...
  const processedContent = useMemo(() => {
    ...
    return contentStr.replace(h2Regex, (match, attrs, text) => {
      ...
      // Clean attributes to avoid duplication of id or class
      let cleanAttrs = attrs.replace(/id=["']([^"']+)["']/i, "").trim();
      cleanAttrs = cleanAttrs.replace(/class=["']([^"']+)["']/i, "").trim();
      
      return `<h2 id="${id}" class="scroll-mt-24 group flex items-center gap-2" ${cleanAttrs}>${text}</h2>`;
    });
  }, [post?.content]);
  ...
  const faqSchema = useMemo(() => {
    ...
    // Soru ve Cevapları regex ile yakala (örn: <strong>Soru 1: ... </strong> ve <strong>Cevap: ...</strong>)
    const questionRegex = /<strong>Soru\s*\d*:\s*(.*?)<\/strong>[\s\r\n]*<br>[\s\r\n]*<strong>Cevap:\s*(.*?)<\/p>/gi;
  ...
  return (
    <Layout>
      ...
      {/* Premium Hero Design */}
      <header className="relative pt-24 md:pt-36 pb-28 md:pb-36 overflow-hidden border-b border-border/10 bg-slate-950/20">
        ...
        <div className="container mx-auto px-4 max-w-5xl">
          ...
          <div className="flex flex-wrap items-center justify-between gap-6 border-t border-border/30 pt-6">
            ...
            <div className="flex flex-wrap items-center gap-3">
              {/* Metin Boyutu Ayarlayıcı */}
              <div className="inline-flex items-center bg-card/80 backdrop-blur-sm border border-border/60 rounded-full p-1 shadow-sm">
                <span className="text-[10px] font-extrabold text-muted-foreground/80 px-2.5 tracking-wider flex items-center gap-1.5 border-r border-border/40 mr-1">
                  <Type className="w-3.5 h-3.5 text-primary" /> OKUMA BOYUTU:
                </span>
        ...
        <div className="container mx-auto px-4 max-w-6xl">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-start">
            ...
            <aside className="lg:col-span-4 lg:sticky lg:top-24 space-y-8 order-2 lg:order-1">
              ...
              {/* Premium Red/White Service CTA Card */}
              <div className="p-6 rounded-2xl border border-primary/40 bg-gradient-to-br from-primary/10 via-[#120505] to-background text-xs space-y-5 shadow-lg shadow-primary/5 relative overflow-hidden group">
                <div className="absolute top-0 right-0 w-32 h-32 bg-primary/10 rounded-full blur-2xl -z-10 group-hover:bg-primary/20 transition-all duration-500" />
                <div className="flex items-center gap-2 border-b border-border/30 pb-3">
                  <Phone className="w-5 h-5 text-primary shrink-0 animate-pulse" />
                  <span className="font-bold text-foreground text-xs uppercase tracking-wider">Fiyat Teklifi &amp; Randevu</span>
                </div>
                <p className="text-muted-foreground leading-relaxed text-[11px]">
                  Motosikletiniz için periyodik bakım seti fiyatları, orijinal yedek parça temini ve profesyonel servis randevusu için hemen bizimle iletişime geçin.
                </p>
                <a
                  href="https://wa.me/905324956402?text=Merhaba,%20blog%20yazinizdan%20geliyorum.%20Motosikletim%20icin%20yedek%20parca%20ve%20servis%20randevusu%20almak%20istiyorum."
                  target="_blank"
                  rel="noopener noreferrer"
                  className="flex items-center justify-center gap-2.5 w-full py-3.5 px-4 rounded-xl bg-primary hover:bg-primary/90 text-white font-extrabold text-xs uppercase tracking-wider transition-all duration-300 shadow-md shadow-primary/20 hover:shadow-primary/40 active:scale-[0.98] cursor-pointer"
                >
                  <MessageCircle className="w-4 h-4 text-white" /> WhatsApp Destek Hattı
                </a>
              </div>
              ...
              {processedContent && (
                <div className="pasa-article">
                  <ReactMarkdown
                    ...
                    components={{
                      h1: ({node, ...props}) => {
                        const id = (node as any)?.properties?.id || (props as any).id;
                        return <h1 id={id} className={getHeadingSizeClasses(textSize).h1} {...props} />;
                      },
                      h2: ({node, ...props}) => {
                        const id = (node as any)?.properties?.id || (props as any).id;
                        return <h2 id={id} className={getHeadingSizeClasses(textSize).h2} {...props} />;
                      },
                      h3: ({node, ...props}) => {
                        const id = (node as any)?.properties?.id || (props as any).id;
                        return <h3 id={id} className={getHeadingSizeClasses(textSize).h3} {...props} />;
                      },
                      p: ({node, ...props}) => <p className={`${getParagraphSizeClass(textSize)} mb-5 text-muted-foreground`} {...props} />,
                      ul: ({node, ...props}) => <ul className={`list-disc list-inside mb-5 space-y-2 ${getListSizeClass(textSize)} text-muted-foreground`} {...props} />,
                      ol: ({node, ...props}) => <ol className={`list-decimal list-inside mb-5 space-y-2 ${getListSizeClass(textSize)} text-muted-foreground`} {...props} />,
      ...
      {/* Yüzen Okuma Boyutu Kontrolü (Asla WhatsApp Butonuyla Çakışmaz, Mobilde Üstte Asılı, Masaüstünde Sol Kenarda Sabit) */}
      {showFloatingSizeControls && (
        <div className="fixed bottom-24 right-6 md:bottom-auto md:right-auto md:left-6 md:top-1/2 md:-translate-y-1/2 z-50 animate-in fade-in slide-in-from-bottom-4 md:slide-in-from-left-4 duration-300">
          <div className="flex flex-row md:flex-col items-center gap-1.5 bg-slate-950/95 backdrop-blur-md border border-primary/30 p-1.5 md:p-2 rounded-full md:rounded-2xl shadow-[0_10px_35px_-5px_rgba(0,0,0,0.5)]">
            <div className="flex flex-row md:flex-col items-center gap-1.5 px-2.5 py-1 md:py-0 md:pb-2 text-muted-foreground border-r md:border-r-0 md:border-b border-border/40 shrink-0">
              <Type className="w-4 h-4 text-primary" />
              <span className="text-[9px] font-black tracking-wider uppercase text-foreground select-none hidden md:inline">
                BOYUT
              </span>
            </div>
            {(["sm", "base", "lg", "xl"] as const).map((sz) => (
              <button
                key={sz}
                onClick={() => setTextSize(sz)}
                className={`w-8 h-8 md:w-9 md:h-9 rounded-full text-xs font-bold flex items-center justify-center transition-all duration-200 cursor-pointer ${
                  textSize === sz
                    ? "bg-primary text-primary-foreground shadow-md scale-110 font-black"
                    : "text-muted-foreground hover:text-foreground hover:bg-muted/50"
                }`}
                title={sz === "sm" ? "Küçük" : sz === "base" ? "Normal" : sz === "lg" ? "Büyük" : "Çok Büyük"}
              >
                {sz === "sm" ? "A-" : sz === "base" ? "A" : sz === "lg" ? "A+" : "A++"}
              </button>
            ))}
