...
const Galeri = () => {
  ...
  useEffect(() => {
    const fetchImages = async () => {
      setLoading(true);
      try {
        const cached = sessionStorage.getItem("gallery_cache"); const cacheTime = sessionStorage.getItem("gallery_time"); if (cached && cacheTime && Date.now() - parseInt(cacheTime) < 30 * 60 * 1000) { setImages(JSON.parse(cached)); setLoading(false); return; } const { data, error } = await dbClient.from("gallery_images").select("id, url, title, category").order("sort_order").order("created_at", { ascending: false });
        if (error) {
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {filtered.map((img, i) => (
                <div key={i} className="group relative rounded-xl overflow-hidden aspect-video">
                  <ImageWithFallback src={img.src} alt={img.title} width={600} height={400} loading="lazy" className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
