...
export default function AdminNotes() {
  ...
  const loadNotes = async () => {
    ...
    try {
      ...
      if (data) {
        ...
        secureStorage.setItem("pasamotor_admin_notes", JSON.stringify(data));
      } else {
        throw new Error("Boş veri döndü");
      }
    } catch (e) {
      // Bulut bağlantısı veya tablo yoksa yerel kaydı yükle
      const localDataStr = secureStorage.getItem("pasamotor_admin_notes");
      if (localDataStr) {
        try {
        ...
        setNotes(defaults);
        secureStorage.setItem("pasamotor_admin_notes", JSON.stringify(defaults));
      }
      setSyncStatus("local");
    } finally {
      setLoading(false);
  ...
  const saveNotesToSync = async (updatedNotes: Note[]) => {
    ...
    secureStorage.setItem("pasamotor_admin_notes", JSON.stringify(updatedNotes));
                  ...
                  {noteDiagram ? (
                    <div className="space-y-3">
                      ...
                      <div className="relative group/drawing overflow-hidden rounded-xl border border-slate-800 bg-slate-950 aspect-[21/9] flex items-center justify-center p-2">
                        <img src={noteDiagram} alt="Not Çizimi" className="max-w-full max-h-full object-contain opacity-80" loading="lazy" decoding="async" />
