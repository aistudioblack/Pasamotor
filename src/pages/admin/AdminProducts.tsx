...
const AdminProducts = () => {
  ...
  const runBeautifier = async () => {
    setBeautifying(true);
    try {
      const res = await adminFetch("/api/supplier/beautify-supabase", {
        method: "POST"
      });
                ...
                <tbody>
                  {paginatedItems.map((p) => (
                    <tr key={p.id} className="border-t border-border hover:bg-muted/30">
                      ...
                      <td className="p-3">
                        {p.images?.[0] ? (
                          <img src={p.images[0]} alt={p.title} className="w-10 h-10 rounded object-cover" loading="lazy" decoding="async" />
                        ) : (
