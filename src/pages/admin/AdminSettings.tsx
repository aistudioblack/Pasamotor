...
const AdminSettings = () => {
  ...
  const [aiProvider, setAiProvider] = useState<"system" | "openrouter" | "groq" | "gemini" | "huggingface" | "persorai">("system");
  ...
  useEffect(() => {
    const savedProvider = secureStorage.getItem("ai_provider") as any;
    if (savedProvider) {
    ...
    const savedHf = secureStorage.getItem("hf_api_key");
    ...
    try {
      const savedCustomProviders = secureStorage.getItem("custom_ai_providers");
  ...
  const handleSaveAISettings = (e: React.FormEvent) => {
    e.preventDefault();
    try {
      secureStorage.setItem("ai_provider", aiProvider);

      if (!orApiKey.trim()) {
        secureStorage.removeItem("or_api_key");
      } else {
        secureStorage.setItem("or_api_key", encryptKey(orApiKey.trim()));
      }

      if (!groqApiKey.trim()) {
        secureStorage.removeItem("groq_api_key");
      } else {
        secureStorage.setItem("groq_api_key", encryptKey(groqApiKey.trim()));
      }

      if (!geminiApiKey.trim()) {
        secureStorage.removeItem("gemini_api_key");
      } else {
        secureStorage.setItem("gemini_api_key", encryptKey(geminiApiKey.trim()));
      }

      if (!hfApiKey.trim()) {
        secureStorage.removeItem("hf_api_key");
      } else {
        secureStorage.setItem("hf_api_key", encryptKey(hfApiKey.trim()));
      }

      if (!persoraiApiKey.trim()) {
        secureStorage.removeItem("persorai_api_key");
      } else {
        secureStorage.setItem("persorai_api_key", encryptKey(persoraiApiKey.trim()));
      }
      secureStorage.setItem("persorai_model", persoraiModel);
  ...
  const handleDeleteCustomProvider = (id: string) => {
    ...
    secureStorage.setItem("custom_ai_providers", JSON.stringify(encryptedForSave));
          ...
          <form onSubmit={handleSaveAISettings} className="space-y-4">
            <div>
              ...
              >
                ...
                <option value="puter">Puter.js (Free & Unlimited AI API - Limitsiz & Ücretsiz)</option>
            ...
            {(aiProvider as any) === "puter" ? (
              <div className="p-4 rounded-xl bg-green-950/20 border border-green-500/20 text-xs text-muted-foreground space-y-1.5 leading-relaxed">
                <span className="font-bold text-green-400 block font-sans">🚀 Puter.js Otonom Web OS Yapay Zekası</span>
                <p className="font-sans text-[11px]">
                  Bu mod seçildiğinde, dünyanın ilk tarayıcı tabanlı bulut işletim sistemi olan Puter'ın <strong>ücretsiz ve sınırsız</strong> yapay zeka modelleri (GPT-4o, Claude 3.5 Sonnet, Gemini 1.5) arka planda otomatik kullanılır.
                </p>
                <p className="text-[10px] text-green-300/80 font-sans italic">
                  * API Anahtarı girmeniz veya herhangi bir kurulum yapmanız gerekmez. Anında kullanıma hazırdır.
                </p>
              </div>
            ) : aiProvider === "system" ? (
