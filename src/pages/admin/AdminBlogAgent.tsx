...
import { translateTitleToEnglishPrompt } from "@/lib/blogPromptHelper";
import { secureStorage } from '../../lib/secure-storage';

// Human Refine: Strip AI fillers, sanitize characters, and add Paşa Motor Teknik Heyeti disclaimer
...
export default function AdminBlogAgent() {
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      if (err.message === "DOMAIN_NOT_AUTHORIZED") {
        addLog("🚨 Firebase Domain Yetkilendirme Hatası: Bu domaine Google Girişi için izin verilmemiş.", "error");
        addLog("LÜTFEN ŞU ADIMLARI İZLEYİN: 1) Firebase Konsolunuza gidin 2) Authentication menüsüne tıklayın 3) Settings (Ayarlar) sekmesine geçin 4) Authorized domains listesine şu anki domain adresini (örn. pasamotor.com.tr) ekleyin.", "warning");
        toast({ 
          title: "Domain İzni Gerekli", 
          description: "Firebase konsolundan mevcut domain adresinizi 'Authorized domains' listesine eklemelisiniz.", 
          variant: "destructive" 
        });
...
JSON listesi olarak { "recommendations": [ { "keyword": "ornek", "reason": "neden seçtik...", "search_volume": "1000", "current_ctr": "1.2%" } ] } şeklinde yanıtla. Sadece JSON dönder.`;
  ...
  const isProviderKeyConfigured = (): boolean => {
    ...
    if (provider === "openrouter") {
      const k = secureStorage.getItem("or_api_key");
      return !!k && decryptKey(k).trim().length > 0;
    }
    if (provider === "groq") {
      const k = secureStorage.getItem("groq_api_key");
      return !!k && decryptKey(k).trim().length > 0;
    }
    if (provider === "gemini") {
      const k = secureStorage.getItem("gemini_api_key");
      return !!k && decryptKey(k).trim().length > 0;
    }
    if (provider === "huggingface") {
      const k = secureStorage.getItem("hf_api_key");
      return !!k && decryptKey(k).trim().length > 0;
    }
    if (provider === "persorai") {
      const k = secureStorage.getItem("persorai_api_key");
      return !!k && decryptKey(k).trim().length > 0;
    }
    return false;
  };
  ...
  const systemInstruction = `Sen dünyanın en iyi motosiklet uzmanı editörü, SEO uzmanı ve yapay zeka içerik üreticisisin. Mükemmel Türkçe kullanırsın, motosiklet mekaniği, yedek parçalar, kronik arızalar, tamir atölyesi ortamı ve motosiklet markaları konusunda devasa bir bilgi birikimine sahipsin. Önceki konuşmaları ve ürettiğin içerikleri hatırlar, asla birbirinin aynısı cümleler kurmaz veya benzer makaleler üretmezsin. Her yeni makale eskisinden farklı, detaylı, teknik doğrulukta, gerçekçi tamir senaryolarına dayanan ve organik olmalıdır. Asla motosiklet dünyası dışına çıkma.`;
  ...
  const callOpenRouter = async (
    ...
    const tryOpenRouter = async (customPrompt?: string): Promise<string> => {
      const savedKeyEncrypted = secureStorage.getItem("or_api_key");
    ...
    const tryGroq = async (customPrompt?: string): Promise<string> => {
      const savedKey = secureStorage.getItem("groq_api_key");
    ...
    const tryGemini = async (): Promise<string> => {
      const savedKey = secureStorage.getItem("gemini_api_key");
    ...
    const tryHuggingFace = async (): Promise<string> => {
      const savedKey = secureStorage.getItem("hf_api_key");
    ...
    const tryPersorAI = async (): Promise<string> => {
      const savedKey = secureStorage.getItem("persorai_api_key");
      if (!savedKey) throw new Error("⚠️ PersorAI API Key eksik!");
      const realKey = decryptKey(savedKey);

      // Model seçimi
      const savedModel = secureStorage.getItem("persorai_model") || "claude-opus-4-7";
    ...
    const getProvidersForTask = (task?: "keyword" | "image_prompt" | "analysis" | "outline" | "article" | "synthesis" | "general"): string[] => {
      ...
      let baseOrder = ["system", "persorai", "openrouter", "groq", "gemini"];
      ...
      if (task === "keyword" || task === "image_prompt") {
        ...
        baseOrder = ["system", "groq", "gemini", "persorai", "openrouter"];
        ...
        baseOrder = ["system", "persorai", "gemini", "openrouter", "groq"];
      } else if (task === "outline") {
        // Yapısal hiyerarşi oluşturma: Sistem ve Groq/OpenRouter dengesi.
        baseOrder = ["system", "persorai", "groq", "openrouter", "gemini"];
        ...
        baseOrder = ["persorai", "groq", "openrouter", "system", "gemini"];
      } else if (task === "synthesis") {
        // En karmaşık akıl yürütme (Ajan 3): PersorAI, OpenRouter (R1), Gemini Pro.
        baseOrder = ["persorai", "openrouter", "system", "gemini", "groq"];
      }
  ...
  const generateImagePromptWithQwen = async (title: string, style: string): Promise<string> => {
    try {
      const styleText = style.includes("cinematic") ? "cinematic photography, ultra-realistic, 8k resolution, highly detailed, shot on 35mm lens, sharp focus" :
                        style.includes("3d") ? "hyper-realistic 3d render, unreal engine 5, octane render, raytracing, highly detailed" :
                        style.includes("studio") ? "studio lighting, dark premium clean background, sleek product photography, macro shot" :
                        "clean hyper-realistic illustration, highly detailed, professional lighting";
      
      const geminiPrompt = `Act as a Senior Visual Content Manager. Create an ultra-detailed image generation prompt for a photorealistic blog cover image for a post titled: "${title}".
The prompt MUST BE IN ENGLISH ONLY. Use descriptive, comma-separated keywords.

CRITICAL INSTRUCTIONS FOR MAXIMUM REALISM AND QUALITY:
1. Scene & Environment: Describe a highly realistic, professional, and well-lit motorcycle repair workshop or a specific, hyper-realistic close-up of motorcycle parts on a clean workbench. No chaotic, messy, or nonsensical AI artifacts. The setting must look like an actual premium service center.
2. Mandatory Branding: The image MUST feature the exact words "PAŞA MOTOR" clearly and flawlessly integrated into the scene. For example, a clear sign on the workshop wall saying "PAŞA MOTOR", a large printed metal sign above the entrance, or a clean professional logo on a workbench or toolbox.
3. Relevant Details: The visual MUST directly reflect the title. If it's about an engine, show pristine engine parts. If about oil, show a clear oil bottle with a funnel. If about a specific motorcycle, describe that exact motorcycle model accurately.
4. Photographic Quality: ${styleText}. Specify camera angles (e.g., shallow depth of field, macro shot, wide angle workshop view), lighting (e.g., natural sunlight from a window, bright LED shop lights), and quality (e.g., photorealistic, 8k, award-winning photography).
5. Zero Gibberish & Plain Clothing: DO NOT INCLUDE ANY OTHER ENGLISH TEXT OR GIBBERISH TEXT IN THE IMAGE. The only allowed text is "PAŞA MOTOR" on a workshop wall or sign. Crucially, the overalls, uniforms, or clothes of any mechanics or workers shown in the image MUST BE COMPLETELY SOLID, PLAIN, AND UNBRANDED WITH NO LOGOS, NO BADGES, AND NO TEXT WHATSOEVER.

Output ONLY the raw image generation prompt without any conversational text, quotes, or markdown. Make sure the prompt is optimized for an advanced image generator.`;
      
      // Geçici olarak sağlayıcıyı system (Google Gemini) yapıp çağırıyoruz
      const oldProvider = secureStorage.getItem("ai_provider");
      secureStorage.setItem("ai_provider", "system");
      
      const res = await callOpenRouter(geminiPrompt, false, "image_prompt");
      
      if (oldProvider) secureStorage.setItem("ai_provider", oldProvider);
      else secureStorage.removeItem("ai_provider");
  ...
  // ================= KONTROL KATMANI: KALİTE PARAMETRELERİ & OPTİMİZE KAYIT =================
  const generateAndSaveOptimizedCover = async (rawPrompt: string, articleTitle: string): Promise<string> => {
    addLog(`🛡️ [Kontrol Katmanı] Görsel Gemini API ile üretiliyor...`, "info");
    
    try {
      // Use the newly created helper function
      const { generateBlogImage } = await import("@/lib/gemini-image");
      const { url, error } = await generateBlogImage(articleTitle, Date.now().toString(), rawPrompt);
      
      if (url) {
        addLog("🎉 [Başarılı] Gemini görseli 'assets/images/blogimg' klasörüne kaydedildi!", "success");
        return url;
      } else {
        addLog(`⚠️ Görsel üretilemedi: ${error}`, "warning");
        return "";
      }
    } catch (e: any) {
      addLog(`❌ Görsel hatası: ${e.message}`, "error");
      return "";
    }
  };

  // Görsel boyutunu canvas ile optimize edip (1024x576 WebP) Supabase 'assets/images/blogimg' klasörüne yükleme
  const processAndUploadBlogCover = async (blob: Blob, title: string): Promise<string> => {
    return new Promise((resolve) => {
      ...
      img.onload = async () => {
        ...
        canvas.width = 1024;
        canvas.height = 576; // 16:9 Blog standart kapak boyutu
        const ctx = canvas.getContext("2d");
        if (!ctx) { resolve(""); return; }
        
        ctx.drawImage(img, 0, 0, 1024, 576);
        const base64Data = canvas.toDataURL("image/webp", 0.85); // Optimize edilmiş %85 kalite WebP
        ...
        const fileName = `images/blogimg/${cleanSlug}-${Date.now()}.webp`;
        ...
        try {
          ...
          if (res.ok) {
            ...
            resolve(data.publicUrl || data.imageUrl || data.url || base64Data);
          } else {
            resolve(base64Data);
          }
        } catch(e) {
  ...
  const runAutoPilotWorkflow = async () => {
    ...
    try {
      ...
      if (activeUrls.length > 0) {
        ...
        const crawlerRes = await adminFetch("/api/competitor-analysis", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
      ...
      // Kontrol Katmanı ile %100 Çalışan Yapay Zeka Kapak Görseli Üretimi ve Kaydı
      const autoCoverUrl = await generateAndSaveOptimizedCover(seoImagePrompt, parsedArticle.title);
  ...
  const proceedToImageGeneration = async () => {
    ...
    try {
      ...
      const finalUrl = await generateAndSaveOptimizedCover(promptSeed, article.title);
  ...
  const regenerateCoverImage = async () => {
    if (!coverPrompt.trim() || !article) return;
    setIsGeneratingImage(true);
    ...
    try {
      ...
      const newUrl = await generateAndSaveOptimizedCover(fullPrompt, article.title);
