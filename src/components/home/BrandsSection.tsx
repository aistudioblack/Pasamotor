      ...
      <div className="container mx-auto px-4 relative z-10">
        <div className="text-center max-w-2xl mx-auto mb-12">
          ...
          <p className="text-muted-foreground text-sm md:text-base leading-relaxed">
            İstanbul Fatih'te <strong className="text-foreground">TVS</strong>, <strong className="text-foreground">Falcon</strong>, <strong className="text-foreground">Işıldar</strong> ve <strong className="text-foreground">Moto Lux</strong> markalarının resmî yetkili satış, orijinal yedek parça ve teknik servis noktası.
          </p>
        </div>

        {/* Marka Kartı Grid - Yan yana sığacak şekilde ölçeklendirildi */}
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 lg:gap-6 max-w-6xl mx-auto justify-center items-stretch">
          {authorizedBrands.map((brand) => {
            return (
              <Link
                key={brand.slug}
                to={`/marka/${brand.slug}`}
                className="group relative overflow-hidden bg-[#111318]/90 hover:bg-[#161318] border border-white/[0.08] hover:border-red-500/40 rounded-2xl p-4 md:p-6 flex flex-col items-center justify-between text-center transition-all duration-300 cursor-pointer shadow-lg hover:shadow-[0_12px_35px_-10px_rgba(239,68,68,0.3)] hover:-translate-y-1.5 min-h-[260px] w-full"
              >
                ...
                <div className="w-full flex justify-center relative z-10">
                  <span className="inline-flex items-center text-[9px] md:text-[10px] uppercase font-bold tracking-wider px-2.5 py-1 rounded-full bg-red-500/10 text-red-400 border border-red-500/20 group-hover:bg-red-500 group-hover:text-white transition-colors duration-300 shadow-sm whitespace-nowrap">
                    {brand.badge || "Yetkili Servis"}
                  </span>
                </div>

                {/* Orta Kısım: Logo Kutusu */}
                <div className="relative w-20 h-20 md:w-24 md:h-24 my-4 rounded-2xl bg-[#181b22] border border-white/[0.1] flex items-center justify-center overflow-hidden transition-all duration-300 group-hover:scale-105 group-hover:bg-[#21161a] group-hover:border-red-500/30 shadow-inner">
                  ...
                  <span className="font-heading font-black text-lg md:text-xl tracking-tight text-foreground group-hover:text-red-400 transition-colors duration-300 relative z-10 break-words w-full px-2">
                    {brand.name}
                  </span>
                </div>
                ...
                <div className="relative z-10 flex flex-col items-center w-full mt-auto">
                  <h3 className="font-heading font-bold text-sm md:text-base text-foreground group-hover:text-red-400 transition-colors duration-300 mb-1 text-center line-clamp-1">
                    {brand.name} Motosiklet
                  </h3>
                  <p className="text-muted-foreground text-[10px] md:text-xs leading-relaxed max-w-[220px] mb-3 line-clamp-2 md:line-clamp-3">
                    {brand.desc}
                  </p>

                  <span className="inline-flex items-center gap-1.5 text-[10px] md:text-xs font-semibold text-red-400 group-hover:text-red-300 transition-all duration-300 whitespace-nowrap">
                    Detay
