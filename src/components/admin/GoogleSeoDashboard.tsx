...
export default function GoogleSeoDashboard() {
  ...
  const [ga4PropertyId, setGa4PropertyId] = useState(() => localStorage.getItem("ga4_property_id") || "538491562");
      ...
      async (user, accessToken) => {
        ...
        try {
          ...
          if (data && data.sections) {
            ...
            if (secs.gscSiteUrl && !localStorage.getItem("gsc_site_url")) {
              setGscSiteUrl(secs.gscSiteUrl);
              localStorage.setItem("gsc_site_url", secs.gscSiteUrl);
            }
            if (secs.ga4PropertyId && !localStorage.getItem("ga4_property_id")) {
              setGa4PropertyId(secs.ga4PropertyId);
              localStorage.setItem("ga4_property_id", secs.ga4PropertyId);
            }
          }
        } catch(e) {
  ...
  const handleSaveCustomSettings = async (targetGsc: string, targetGa: string) => {
    localStorage.setItem("gsc_site_url", targetGsc.trim());
    localStorage.setItem("ga4_property_id", targetGa.trim());
  ...
  const handleLog = (text: string, type: "info" | "success" | "warning" | "error" = "info") => {
    ...
    setSystemLogs(prev => {
      ...
      try {
        localStorage.setItem("admin_sys_logs", JSON.stringify(updated));
      } catch(e) {
  ...
  const loadSystemLogs = () => {
    try {
      const saved = localStorage.getItem("admin_sys_logs");
      if (saved) {
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      if (result) {
        ...
        toast({ title: "Google OAuth Bağlandı", description: "Google credentials Supabase üzerine güvenli bir şekilde kaydedildi.", variant: "default" });
        handleLog(`Google OAuth login başarılı. Hesap: ${result.user.email}. Jeton Supabase'e kaydedildi.`, "success");
      ...
      if (err?.code !== 'auth/popup-closed-by-user') {
        ...
        handleLog("Google OAuth yetkilendirmesi reddedildi veya başarısız oldu.", "error");
      }
    } finally {
  ...
  const loadData = async (accessToken: string) => {
    ...
    const currentGscUrlSetting = (localStorage.getItem("gsc_site_url") || gscSiteUrl || "").trim();
    const currentGaPropertySetting = (localStorage.getItem("ga4_property_id") || ga4PropertyId || "538491562").trim();
      ...
      {!needsAuth && (
        <div className="glass-card rounded-xl p-6 border border-border/40 bg-gradient-to-br from-primary/5 to-amber-500/5">
          <h2 className="font-heading font-semibold text-foreground text-sm flex items-center gap-2 mb-4">
            <Sparkles className="w-4 h-4 text-amber-400" /> Gemini & Manus AI Ortak Akıl SEO ve Envanter Kararları
          </h2>
